...
export default function Customers() {
  ...
  const customerName = (c) => c.name || 'Unknown';
  const customerPhone = (c) => c.phone || '\u2014';
  const customerEmail = (c) => c.email || '\u2014';
  const customerCity = (c) => c.city || '\u2014';
  const customerOrders = (c) => c.totalOrders ?? c.ordersCount ?? 0;
  const customerSpent = (c) =>
    (c.totalSpent ?? c.lifetimeValue ?? c.totalValue ?? 0).toFixed(2);
  const customerLastMsg = (c) => c.lastMessage?.body || c.lastMessage?.text || c.lastMessage || '';
  const customerCreatedAt = (c) => c.createdAt || c.created || '';
              ...
              <div>
                ...
                <div className="p-5 space-y-6">
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-16 h-16 rounded-full flex items-center justify-center text-white text-2xl font-bold flex-shrink-0 ${getAvatarColor(detail.name || '')}`}
                    >
                      {getInitials(detail.name || '')}
                    </div>
                    <div>
                      <h3 className="text-xl font-bold text-gray-900">{detail.name || 'Unknown'}</h3>
                      <p className="text-sm text-gray-500">{detail.phone || 'No phone'}</p>
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-gray-900 flex items-center gap-1">
                          ...
                          {formatDate(detail.createdAt || detail.created) || '\u2014'}
                        </p>
                      </div>
                      {detail.address && (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="card p-4 text-center">
                      ...
                      <p className="text-2xl font-bold text-gray-900">
                        {detail.totalOrders ?? detail.ordersCount ?? 0}
                      </p>
                    ...
                    <div className="card p-4 text-center">
                      ...
                      <p className="text-2xl font-bold text-gray-900">
                        ${(detail.totalSpent ?? detail.lifetimeValue ?? detail.totalValue ?? 0).toFixed(2)}
                      </p>
                      <p className="text-xs text-gray-500">Total Spent</p>
                    </div>
                  </div>

                  {(detail.orderHistory || detail.orders) && (
                    <div className="card p-4 space-y-3">
                      <h3 className="text-sm font-semibold text-gray-900 flex items-center gap-2">
                        <ShoppingCart className="w-4 h-4 text-gray-400" />
                        Order History
                      ...
                      {(() => {
                        const orders = detail.orderHistory || detail.orders || [];
                  ...
                  {(detail.messageHistory || detail.messages) && (
                    <div className="card p-4 space-y-3">
                      <h3 className="text-sm font-semibold text-gray-900 flex items-center gap-2">
                      ...
                      {(() => {
                        const messages = detail.messageHistory || detail.messages || [];
                        ...
                        return (
                          <div className="space-y-2">
                            {messages.slice(0, 10).map((msg, i) => {
                              ...
                              const direction = msg.direction || msg.type || 'inbound';
                              return (
                                <div
                                  key={msg._id || msg.id || i}
                                  className={`rounded-xl p-3 text-sm max-w-[85%] ${
                                    direction === 'outbound' || direction === 'employee'
